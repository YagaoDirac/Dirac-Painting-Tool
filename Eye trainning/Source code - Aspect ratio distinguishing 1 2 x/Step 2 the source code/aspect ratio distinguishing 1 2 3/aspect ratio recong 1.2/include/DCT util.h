...
public:
	...
	{
		return DCT_rand_choose(src, p_result);
	}
...
Numeric_type DCT_lerp(const Numeric_type& a, const Numeric_type& b, float t)
{
	return a * (1 - t) + b * t;
}

template<typename T>
const T& DCT_max(const T& a, const T& b) { return a > b ? a : b; }
template<typename T>
const T& DCT_min(const T& a, const T& b) { return a < b ? a : b; }
template<typename Numeric_type>
const Numeric_type& DCT_clamp(const Numeric_type& a, const Numeric_type& min, const Numeric_type& max)
{ 
	return DCT_max(min, DCT_min(a, max));
}
