...
//step 1
void DET_aspect_ratio_trainer_data::DET_auxiliary_line_data_nested_class::push_raw(float in_ratio, unsigned int in_half_len)
{
	data.push_back(auxiliary_line_info_struct{ in_ratio,in_half_len,-1 });
}
//step 2
void DET_aspect_ratio_trainer_data::DET_auxiliary_line_data_nested_class::update(int in_question_top, int in_question_width, filling_mode_enum in_mode)
{
...
void DET_aspect_ratio_trainer_data::DET_auxiliary_line_data_nested_class::clear() { data.clear(); };

//how to 
void DET_aspect_ratio_trainer_data::DET_auxiliary_line_data_nested_class::how_to_inside()
{
	return;
	/*
...
DET_aspect_ratio_trainer_data::error_type DET_aspect_ratio_trainer_data::generate_to_image_data_member_stage_1()
{
	...
	//prepares the options for answer space.
	{
		float lower_boundary = answer_manager.get_lower();
		float upper_boundary = answer_manager.get_upper();
		float division_value = answer_manager.get_division();
		answer_options.clear();
		for (float r = lower_boundary; r <= upper_boundary + 0.001f; r += division_value)
			answer_options.push_back(r);
	}
	//Finally randomly picks one.
