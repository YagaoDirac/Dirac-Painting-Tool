...
function window_onload()
{
...

   {
      ...
      aboutText.appendChild(document.createTextNode("Line counting 1.0.0. Dirac's painting tool. Github.com/yagaodirac/Dirac-painting-tool"))
   }
}
   ...
   switch (data.taskToDo) {
      case "question":
         ...
         //Draws the question dot.
         {
            ...
            //__DEGUB__
            if (false) {
               context.fillStyle = "rgb(255,1,255)";
               context.fillRect(startPoint.x - 10, startPoint.y - 10, 20, 20);
				}
...
function getDifferentColor2(ori1, ori2, distance) {
   ...
   if (rd < count1) return s1 + rd;
   if (rd < (count1+count2))return s2 + (rd - count1);
   return s2 + (rd - count1 -count2);
}